const TARGET_URL = 'https://corsproxy.io?' + encodeURIComponent('https://google.com');
...
function parseFormCSV(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (!houseDatabase[aptNum]) houseDatabase[aptNum] = [];
        ...
        namesArray.forEach((name, index) => {
            ...
            if (phone.trim().replace(/[^\d+]/g, '').length === 9 && !phone.startsWith('+')) {
                phone = '+375' + phone.trim().replace(/[^\d+]/g, '');
            }
            houseDatabase[aptNum].push({ fio: cleanName, phone: phone, meta: `Запись от: ${row[0]}` });
        });
    }
}
    ...
    apartments.forEach(apt => {
        ...
        if (matchesApt || matchesName) {
            ...
            if (window.innerWidth < 768) {
                if (aptNum <= 44) switchEntrance(1); else switchEntrance(2);
            }
        } else {
...
function buildHouseGrid(containerId, startingApt, entranceId) {
    ...
    for (let currentFloor = 11; currentFloor >= 1; currentFloor--) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            aptDiv.innerHTML = `<div class="apt-num">${aptNumber}</div><div class="apt-count">${isEmpty ? 'пусто' : residents.length + ' чел.'}</div>`;
...
function openInfoPanel(aptNum, floor, entrance, residents) {
    ...
    if (residents.length === 0) {
        container.innerHTML = '<div style="color: #94a3b8; text-align:center; padding-top:30px;">Нет ответов.</div>';
    } else {
        residents.forEach(person => {
            const card = document.createElement('div');
            card.className = `resident-card ${person.fio.trim().split(/\s+/).length === 1 ? 'no-name' : ''}`;
            let phoneField = person.phone ? `<a href="tel:${person.phone}" class="res-phone">📞 ${person.phone}</a>` : `<span style="color: #64748b;">🚫 Нет телефона</span>`;
            card.innerHTML = `<div class="res-name">${person.fio}</div>${phoneField}<div class="res-meta">ℹ️ ${person.meta}</div>`;
            container.appendChild(card);
        });
    }
    darkOverlay.style.display = 'block';
    setTimeout(() => { darkOverlay.classList.add('active'); infoPanel.classList.add('active'); }, 10);
}
...
function initHouse() { buildHouseGrid('entrance1', 1, 1); buildHouseGrid('entrance2', 45, 2); }
...
window.addEventListener('resize', () => {
    if (window.innerWidth >= 768) {
        ...
        switchEntrance(document.getElementById('tab2').classList.contains('active') ? 2 : 1);
    }
});
